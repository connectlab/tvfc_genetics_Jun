...
bhv_cov_data = read_excel(paste0(dir_genotyping, "/your_snp_data_file.xlsx"), sheet = "setting") 
brain_data = bhv_cov_data[,c(5:6)]
cov_data = bhv_cov_data[, c(2:4, 11:20)]
bhv_data = bhv_cov_data[,c(7:10)]
...
data = data.frame(demo_info_select, brain_data, bhv_data, cov_data, snp_data)
# view(data)
...
## model building
################################################
BHV_list = names(bhv_data) #c("Language", "Impulsivity", "CogControl", "Memory")
...
for (BHV in BHV_list) {
  for (model_name in names(select_snps_list)){
    # full model formula
    formula_str = paste(BHV, " ~ ", paste(select_covs_list[[model_name]], collapse = " + "), " + ", paste(select_snps_list[[model_name]], collapse=" + "), sep = "")
    mdl_full_formulas[[paste0(BHV, "_", model_name)]] = as.formula(formula_str)
    
    # full model fitting
    mdl = lmekin(mdl_full_formulas[[paste0(BHV, "_", model_name)]], data = data, varlist = kmat * 2)
    assign(paste0("mdl_full_", BHV, "_", model_name), mdl)
    
    rm(list = c("formula_str", "mdl"))
  }
}
...
snps_counts <- sapply(mdl_full_formulas, function(formula) {
  words <- unlist(strsplit(deparse(formula), "\\s+"))
  rs_count <- sum(grepl("^rs\\w+", words))
  rs_count
})
...
# Generate the covariates-only model formulas
for (BHV in BHV_list) {
  for (model_name in names(select_covs_list)){
  # base model formula
  formula_str <- paste(BHV, "~", paste(select_covs_list[[model_name]], collapse = " + "), sep = "")
  mdl_cov_formulas[[paste0(BHV, "_", model_name)]] <- as.formula(formula_str)
  
  # base model fitting
  mdl = lmekin(mdl_cov_formulas[[paste0(BHV, "_", model_name)]], data = data, varlist = kmat * 2)
  assign(paste0("mdl_cov_", BHV, "_", model_name), mdl)
...
for (model_name in names(mdl_full_formulas)){
  BHV = gsub(pattern = paste(c(DV_list, NT_list), collapse = "|"), "", model_name) # Remove the patterns from NT_list in model_name using gsub
  BHV = sub("__$", "", BHV) # Remove any trailing underscore at the end of DV_norm
